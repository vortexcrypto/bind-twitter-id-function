...
impl ContainerParams {
    pub fn decode(container_params: &Vec<u8>) -> std::result::Result<Self, SwitchboardClientError> {
        ...
        let mut twitter_user_id: String = String::default();
        ...
        for env_pair in params.split(',') {
            ...
            if pair.len() == 2 {
                match pair[0] {
                    ...
                    "TWITTER_USER_ID" => twitter_user_id = String::from_str(pair[1]).unwrap(),
                    _ => {}
                }
            }
        }
        ...
        if twitter_user_id == String::default() {
            return Err(SwitchboardClientError::CustomMessage(
                "TWITTER_USER_ID cannot be undefined".to_string(),
            ));
        }
...
mod tests {
    ...
    fn test_params_decode() {
        let request_params_string = format!(
            "PID={},REALM_PDA={},USER={},USER_ACCOUNT_PDA={},TWITTER_USER_ID={}",
            anchor_spl::token::ID,
            anchor_spl::token::ID,
            anchor_spl::token::ID,
            anchor_spl::token::ID,
        ...
        assert_eq!(params.twitter_user_id, "@bigg_ass_shadow_xxx");
    }
}
